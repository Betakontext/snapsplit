...
class SNAP_PT_panel(Panel):
    ...
    def draw(self, context):
        # Note: Keep structure; only replace texts with tr()
        layout = self.layout
        ...
        header.label(text=tr("ui.section.segmentation", "Segmentation"), icon='MOD_BOOLEAN')
        more_txt = tr("ui.more_less_less", "Less...") if props.ui_more_seg else tr("ui.more_less_more", "More...")
        ...
        box = layout.box()
        header = box.row(align=True)
        header.label(text=tr("ui.section.connections", "Connections"), icon='SNAP_FACE')
        more_txt = tr("ui.more_less_less", "Less...") if props.ui_more_conn else tr("ui.more_less_more", "More...")
        ...
        col.operator("snapsplit.add_connectors",
                     icon="SNAP_FACE",
                     text=tr("op.connectors.add", "Add connectors"))
        col.operator("snapsplit.place_connectors_click",
                     icon="CURSOR",
                     text=tr("op.connectors.place_click", "Place connectors (click)"))
        ...
        more_txt = tr("ui.more_less_less", "Less...") if props.ui_more_tol else tr("ui.more_less_more", "More...")
        ...
        more_txt = tr("ui.more_less_less", "Less...") if props.ui_more_align else tr("ui.more_less_more", "More...")
        ...
        if props.ui_more_align:
            ...
            status_a = (f"A: {nameA} [#{idxA}]" if nameA and idxA >= 0
                        else tr("ui.a_none", "A: none"))
            status_b = (f"B: {nameB} [#{idxB}]" if nameB and idxB >= 0
                        else tr("ui.b_none", "B: none"))
            ...
            row_align.operator("snapsplit.align_faces",
                            text=tr("op.align.label", "Align Faces"),
                            icon='SNAP_ON')
